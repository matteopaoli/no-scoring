...
test.describe("Login Page End-to-End Tests", () => {
  ...
  test("should display login form elements", async ({ page }) => {
    ...
    await expect(page.getByTestId("ad-email-field")).toBeVisible();
    await expect(page.getByTestId("ad-password-field")).toBeVisible();
    await expect(page.getByTestId("ad-login-button")).toBeVisible();

    await page.getByTestId("ad-email-field").click();
    await page.getByTestId("ad-email-field").fill("admin@example.com");
    await page.getByTestId("ad-email-field").press("Tab");
    await page.getByTestId("ad-password-field").fill("secret12");
    await page.getByTestId("ad-login-button").click();
  });
  ...
  test("should show error for invalid credentials", async ({ page }) => {
    await page.getByTestId("ad-email-field").fill("admin@example.com");
    await page.getByTestId("ad-password-field").fill("wrong_password");
    await page.getByTestId("ad-login-button").click();
  ...
  test("should mask the password input", async ({ page }) => {
    await expect(await page.getByTestId("ad-password-field")).toHaveAttribute(
      "type",
      "password"
    );
  });
  ...
  test("should not allow navigation back to login page after successful login", async ({ page }) => {
    ...
    await page.waitForURL("http://localhost:3000/admin");
    await page.goto("http://localhost:3000/admin/login");
    await expect(page.url()).not.toContain("/admin/login");
    await expect(page.url()).toContain("/admin");
  });
});
