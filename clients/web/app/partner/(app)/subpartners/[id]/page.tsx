...
export default async function AgentDetailPage({
  ...
  const stores = await Store.getStores(await PartnerService.getStoresNetwork(agent.id));
  ...
  const totalEarnings = merged.reduce((acc, current) => acc += current.totalCommission, 0)
